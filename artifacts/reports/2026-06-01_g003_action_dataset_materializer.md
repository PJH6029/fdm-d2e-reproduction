...
- Added `scripts/materialize_fdm1_action_dataset.py`, a CLI that reads one or more decoded D2E JSONL record files and writes:
  - `action_slots.jsonl`
...
- Added `configs/data/fdm1_action_dataset.yaml` to pin the intended full-corpus materialization command shape from `outputs/data/d2e_full_corpus/all_records.jsonl` to `outputs/data/fdm1_action_slots`.

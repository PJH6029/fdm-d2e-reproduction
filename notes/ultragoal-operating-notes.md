...
- Current G005 active path is an auxiliary action-prior candidate. Pre-hardening pod run `266056` is CPU/IO-only and must be relaunched after the 16-worker prediction/target-link hardening because the old output target link covered only the first target shard and would fail the G005 completion audit.

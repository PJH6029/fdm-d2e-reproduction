...
- G005 aux example builder exists (`scripts/build_g005_aux_examples.py`, `artifacts/aux/g005_aux_examples_summary.json`) and currently supports the `atari_head_zip_csv_action_adapter` for Atari HEAD Zenodo raw-zip/action-CSV layout with source-specific train/val/test JSONL outputs. G005 completion now requires this summary to pass for all selected aux sources, so MineRL zip and p-doom ArrayRecord adapters must be implemented before G005 can complete.

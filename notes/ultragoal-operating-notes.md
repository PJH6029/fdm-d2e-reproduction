...
## 2026-05-29 KST — Next G005 branch: per-family candidate preservation diagnostics

- Implemented a recipe-faithful continuation from the PAD-aware temporal masked-diffusion IDM after token-presence blending regressed: center-token candidate generation can now preserve a bounded minimum of keyboard, mouse-button, and mouse-move candidates per row before held-out-train family-budget calibration.
- New config: `configs/model/idm_temporal_masked_diffusion_d2e_luma_window5_family_candidates_prefix80k.yaml`. It keeps PAD-aware sparse action-token slots, compact luma video tokens, noncausal temporal masked action-token diffusion, event auxiliaries, train-only retrieval priors, and iterative confidence unmasking; the new candidate preservation prevents frequent mouse-motion tokens from truncating rare key/button candidates before calibration.
- Added diagnostic-only candidate-family summaries for calibration rows and an optional target prefix. These summaries may use ground-truth labels to audit exact candidate coverage/ranks, but they never mutate thresholds or predictions. Do not checkpoint G005 until the bounded H200 probe beats D2E paper targets; this branch is only the next probe candidate.

## 2026-05-29T04:50 KST — G005 family-candidate temporal probe is negative but diagnostic signal improved

- Ran commit/config `2357ce7` / `configs/model/idm_temporal_masked_diffusion_d2e_luma_window5_family_candidates_prefix80k.yaml` on 1×H200 reservation `rsv-jeonghunpark-20260529-14795e`; copied evidence locally and cancelled both the primary reservation and the 05:00 continuation guard `rsv-jeonghunpark-20260529-a4aecc`. W&B run: `https://wandb.ai/pjh6029-seoul-national-university/fdm-d2e-reproduction/runs/rvdr6nic`.
- Evidence prefix: `artifacts/idm/g005_idm_temporal_masked_diffusion_luma_window5_family_candidates_prefix80k_h200_*`; compact summary status is `nonterminal_negative_probe`.
- Result: target-prefix metrics remain far below paper targets (`keyboard_accuracy=0.013898229096615108`, `mouse_button_accuracy=0.007285974499089253`, `mouse_button_f1=0.012578616352201259`, `mouse_move_pearson_x=-0.03688650897554219`, `mouse_move_pearson_y=0.004691930168536975`), although no-button FPR is within the user cap (`0.06588429071443277`). Do **not** checkpoint G005.
- Diagnostic value: per-family candidate preservation proved candidate availability is no longer the bottleneck on the 2k target diagnostic prefix: mouse-button positive rows with exact candidate coverage `33/50` (`0.66`) and keyboard `121/489` (`0.2474`). The remaining failure is score/ranking/calibration/semantic selection from available candidates, and mouse movement exact candidate coverage is still `0/1399`. Next recipe-faithful branch should improve candidate scoring/ranking and vectorize post-checkpoint calibration/prediction before another H200 probe.

## 2026-05-29T04:56:00+09:00 KST — Binding course correction: FDM-1 recipe fidelity is global

- Existing renewed ultragoal IDs/status remain unchanged; do not delete or replace them.
- For G005+ evidence, implementation, and checkpointing, success means reproducing the publicly inferable FDM-1 training recipe on D2E: compact/video-compression screen tokens; IDM as noncausal masked diffusion over masked action-token sequences with iterative confidence unmasking; FDM as autoregressive next-action modeling over interleaved frame/action tokens; key press/release, scroll, exponentially binned mouse deltas, and click/trajectory-style action tokens.
- Novel approaches are allowed only as faithful approximations of unpublished FDM-1 internals on D2E and must be documented as open recipe approximations, never as FDM-1 parity.
- Old heuristic/table/supervised candidates are diagnostic unless rebuilt behind FDM-1 recipe-alignment gates. G005 remains in progress until D2E paper G-IDM targets are beaten by D2E-trained FDM-1-style artifacts.

## 2026-05-29T05:27:00+09:00 KST — G005 family-prior temporal masked-diffusion probe is negative

- Ran commit/config `98d1212` / `configs/model/idm_temporal_masked_diffusion_d2e_luma_window5_family_prior_prefix80k.yaml` on 1×H200 reservation `rsv-jeonghunpark-20260529-1adb50`; copied evidence locally and cancelled the reservation (`after.status=cancelled`). W&B run: `https://wandb.ai/pjh6029-seoul-national-university/fdm-d2e-reproduction/runs/ir5kyqcs`.
...
## 2026-05-29 KST — Next G005 branch: raw-video patch tokens + full-mask curriculum

- Implemented the next FDM-1-public-recipe G005 branch after raw96 global-pooled probes remained negative: temporal masked-diffusion IDM now supports a raw-video patch-token encoder (`video_encoder_arch=raw_video_patch_cnn`) that emits multiple compressed screen-video tokens per temporal row instead of global-pooling each raw 96x96 frame stack into one vector.
- Added `full_action_mask_probability` / `all_action_mask_probability` support in the temporal masked-diffusion dataset. This mixes fully masked action-token windows into training so the model sees the same all-mask start state used by FDM-1-style 16-step iterative IDM inference, while retaining ordinary partial mask/random corruption.
- New bounded probe config/script: `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_allmask_prefix80k.yaml` and `scripts/run_g005_idm_temporal_raw96_patch_allmask_prefix80k.sh`. It keeps D2E raw-video cache inputs, PAD-aware action slots, temporal noncausal masked action-token diffusion, family/token-presence auxiliaries, train-only retrieval priors, and train-heldout calibration; it is not G005 completion evidence until paper targets are beaten and audited.
- Local validation: `uv run pytest -q tests/test_masked_diffusion_idm_trainer.py tests/test_fdm1_recipe_alignment.py` passed and `uv run python scripts/validate_fdm1_recipe_alignment.py` passed. Next step is a fresh H200 prefix probe only after committing/pushing this branch; do not checkpoint G005 unless the paper-target gates pass.

## 2026-05-29T11:34 KST — G005 raw96 patch/full-mask probe is negative

- Ran commit/config `675e981` / `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_allmask_prefix80k.yaml` on production reservation `rsv-jeonghunpark-20260529-adf7db`, pod `prod-rsv-jeonghunpark-20260529-adf7db`, node 4 GPU 1 (visible as one H200). W&B run: `https://wandb.ai/pjh6029-seoul-national-university/fdm-d2e-reproduction/runs/nlphxv37`.
- Evidence prefix: `artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_allmask_prefix80k_h200_*`; compact summary status is `nonterminal_negative_probe`. Reservation was cancelled after evidence copy; the unrelated idle debug reservation `rsv-jeonghunpark-20260529-41d4d0` was also cancelled.
- Result: patch tokens + full-mask curriculum produced nonzero mouse-button F1 (`0.01413`) and improved mouse Pearson over earlier raw96 token-presence (`x=0.02808`, `y=0.04855`), but it badly over-emitted buttons (`no_button_fpr=0.39386`) and remains far from paper targets (`keyboard=0.01268`, `button_acc=0.00897`). Do **not** checkpoint `G005-g014-idm-full-paper-target`.
- Diagnosis: stronger video tokenization and inference-aligned masking improved signal but not calibration; next branch should keep patch/video tokens but reduce false-positive emissions via recipe-faithful no-action/action occupancy modeling or a stricter train-heldout family budget objective that penalizes no-button FPR before target evaluation, not target-label tuning.

## 2026-05-29T11:40 KST — Next G005 branch: action-occupancy gated patch-token IDM

- Implemented the next recipe-faithful branch after the patch/full-mask probe over-emitted mouse buttons (`no_button_fpr=0.39386`): temporal center-candidate scoring now supports multiplicative event/no-action gates from auxiliary heads trained on the same masked-diffusion representation.
- New config/script: `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_gate_prefix80k.yaml` and `scripts/run_g005_idm_temporal_raw96_patch_gate_prefix80k.sh`. It keeps raw96 patch/video tokens and full-mask curriculum, but gates keyboard/button candidates by event-presence probabilities and gates mouse-button candidates by `1 - P(no_button)` from the button-class head before train-heldout family-budget calibration.
- Local validation: `uv run pytest -q tests/test_masked_diffusion_idm_trainer.py tests/test_fdm1_recipe_alignment.py` passed and `uv run python scripts/validate_fdm1_recipe_alignment.py` passed. No GPU reservation is active after the negative patch/full-mask probe; reserve fresh H200 only when ready to test this gated branch.

## 2026-05-29T12:20 KST — G005 raw96 patch-gated IDM probe is negative

- Ran commit/config `f16ebdb` / `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_gate_prefix80k.yaml` on production reservation `rsv-jeonghunpark-20260529-6c0859`, pod `prod-rsv-jeonghunpark-20260529-6c0859`, node 4 GPU 1. W&B run: `https://wandb.ai/pjh6029-seoul-national-university/fdm-d2e-reproduction/runs/2a7zzoq5`.
- Evidence prefix: `artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_gate_prefix80k_h200_*`; compact summary status is `nonterminal_negative_probe`.
- Result: event/no-action gating did **not** fix the patch-token over-emission failure. Observed paper-target metrics on the 5k target prefix: `keyboard_key_accuracy=0.01038`, `mouse_button_accuracy=0.00692`, `mouse_button_f1=0.01301`, `mouse_move_pearson_x=-0.00728`, `mouse_move_pearson_y=null`, and `no_button_false_positive_rate=0.46304`, so all G005 paper-target gates except nonzero button F1 fail. Do **not** checkpoint `G005-g014-idm-full-paper-target`.
- Operational note: the primary reservation was cancelled immediately after terminal evidence appeared, but a tar copy race copied no evidence before pod deletion. A short recovery reservation `rsv-jeonghunpark-20260529-c80d3a` on the same production DDN recovered the terminal artifacts from `/root/work/code/continuous-gui-poc/fdm-d2e-reproduction-g005-gate-f16ebdb` and was cancelled after copy. Both cancellation audits are committed.
- Course-correction implication: this remains FDM-1-public-recipe-shaped diagnostic evidence (raw screen-video patch tokens + video reconstruction + noncausal masked action-token diffusion + 16-step iterative unmasking), but it is not useful enough. Next branch should not add arbitrary heuristics; it should address ranking/alignment inside the FDM-1 recipe, likely by improving video-action temporal alignment and train-only action-token posterior scoring rather than merely gating emissions.

## 2026-05-29T12:30 KST — Next G005 branch: unlabeled-distribution adaptive unmask budget

- Implemented a recipe-faithful inference/calibration branch after fixed train-heldout thresholds over-emitted mouse buttons under target score-distribution shift. The new helper adapts family unmask thresholds to the unlabeled target candidate-score distribution while preserving train-heldout emission budgets and never reading target ground-truth labels.
- New config/script: `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_adaptive_prefix80k.yaml` and `scripts/run_g005_idm_temporal_raw96_patch_adaptive_predict_prefix80k.sh`. This is a prediction-only sweep from the existing raw96 patch-gated checkpoint by default, intended to test FDM-1-style confidence-budgeted iterative unmasking without spending another training run.
- Local validation: `python3 -m py_compile src/fdm_d2e/training/temporal_masked_diffusion_idm_trainer.py scripts/predict_idm_temporal_masked_diffusion.py scripts/train_idm_temporal_masked_diffusion.py`; `bash -n scripts/run_g005_idm_temporal_raw96_patch_adaptive_predict_prefix80k.sh`; `uv run pytest -q tests/test_masked_diffusion_idm_trainer.py tests/test_fdm1_recipe_alignment.py`; `uv run python scripts/validate_fdm1_recipe_alignment.py` all passed. This branch is not G005 completion evidence until an H200 sweep beats paper-target gates.

## 2026-05-29T12:45 KST — G005 adaptive unmask-budget prediction sweep is negative

- Ran commit/config `5031eae` / `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_adaptive_prefix80k.yaml` as a prediction-only H200 sweep from the existing raw96 patch-gated checkpoint on production reservation `rsv-jeonghunpark-20260529-8e2c1e`, pod `prod-rsv-jeonghunpark-20260529-8e2c1e`; reservation was cancelled after evidence copy.
- Evidence prefix: `artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_adaptive_prefix80k_h200_*`; compact summary status is `nonterminal_negative_probe`.
- Result: unlabeled distribution adaptation raised the mouse-button unmask threshold from `0.000437` to `0.355239` using a 400-token target budget and reduced no-button FPR from the prior `0.46304` failure to `0.00371`, but it over-suppressed true button events (`mouse_button_accuracy=0`, `mouse_button_f1=0`) and left keyboard/mouse movement far below paper targets (`keyboard=0.01038`, `mouse_pearson_x=-0.00728`, `mouse_pearson_y=null`). Do **not** checkpoint G005.
- Course-correction implication: score-scale adaptation alone can control FPR but does not solve FDM-1-style action identity/ranking. Next branch must improve video-action alignment / token identity learning inside the masked-diffusion IDM rather than more threshold-only sweeps.

## 2026-05-29T12:52 KST — Next G005 branch: direct auxiliary confidence candidates

- Implemented a recipe-faithful prediction branch after adaptive thresholds proved FPR can be controlled but token identity/ranking remains poor. The temporal masked-diffusion IDM decoder can now inject direct key-token-presence and mouse-button class/presence auxiliary candidates into the iterative unmask candidate set, so auxiliary heads trained on the same noncausal transformer can rank action-token identities instead of only blending noisy slot logits.
- New config: `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_directaux_prefix80k.yaml`. It reuses the raw96 patch-gated checkpoint for a prediction-only sweep by overriding the checkpoint config and remains label-safe: no target-label calibration, only train-heldout budgets plus unlabeled target score adaptation.
- Local validation: `python3 -m py_compile src/fdm_d2e/training/temporal_masked_diffusion_idm_trainer.py scripts/predict_idm_temporal_masked_diffusion.py`; `uv run pytest -q tests/test_masked_diffusion_idm_trainer.py tests/test_fdm1_recipe_alignment.py`; `uv run python scripts/validate_fdm1_recipe_alignment.py` passed. Do not checkpoint G005 unless H200 evidence beats paper gates.

## 2026-05-29T13:15 KST — G005 direct-aux prediction sweep is negative but partially improves ranking

- Ran commit/config `46bc678` / `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_directaux_prefix80k.yaml` as a prediction-only H200 sweep from the existing raw96 patch-gated checkpoint on production reservation `rsv-jeonghunpark-20260529-fec611`, pod `prod-rsv-jeonghunpark-20260529-fec611`; reservation was cancelled after evidence copy.
- Evidence prefix: `artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_directaux_prefix80k_h200_*`; compact summary status is `nonterminal_negative_probe`.
- Result: direct auxiliary candidates improved keyboard over the adaptive-only sweep (`0.01951` vs `0.01038`) and restored nonzero mouse-button F1 (`0.00556`) while keeping no-button FPR under cap (`0.04344`). It still fails paper targets by a large margin: mouse-button accuracy `0.00267`, mouse Pearson X `-0.00447`, Y `null`, keyboard far below `0.73`. Do **not** checkpoint G005.
- Course-correction implication: auxiliary identity ranking has some signal, but the raw96 prefix checkpoint still lacks strong video-action temporal alignment and mouse-motion decoding. Next branch should move beyond prediction-only reranking: either train the direct-aux path explicitly at larger prefix/full scale, add motion-specific patch/trajectory heads within masked diffusion, or revisit the action-tokenization/frame-offset alignment using D2E/FDM-1 public metric constraints.

## 2026-05-29T13:20 KST — Next G005 branch: direct auxiliary mouse-motion tokens

- Implemented a follow-up recipe-faithful prediction branch after direct auxiliary key/button candidates restored some signal but mouse movement remained broken. The decoder can now inject direct mouse-move candidates from the masked action-token presence head, preserving FDM-1-style discrete binned mouse-delta tokens rather than adding an out-of-recipe regression head.
- New config: `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_directaux_motion_prefix80k.yaml`, intended as another prediction-only sweep from the existing raw96 patch-gated checkpoint.
- Local validation: `python3 -m py_compile src/fdm_d2e/training/temporal_masked_diffusion_idm_trainer.py scripts/predict_idm_temporal_masked_diffusion.py`; `uv run pytest -q tests/test_masked_diffusion_idm_trainer.py tests/test_fdm1_recipe_alignment.py`; `uv run python scripts/validate_fdm1_recipe_alignment.py` passed. Do not checkpoint G005 unless H200 evidence beats paper gates.

## 2026-05-29T13:30 KST — G005 direct-aux motion prediction sweep is negative

- Ran commit/config `0c34af1` / `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_directaux_motion_prefix80k.yaml` as a prediction-only H200 sweep from the existing raw96 patch-gated checkpoint on production reservation `rsv-jeonghunpark-20260529-60399f`, pod `prod-rsv-jeonghunpark-20260529-60399f`; reservation was cancelled after evidence copy.
- Evidence prefix: `artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_directaux_motion_prefix80k_h200_*`; compact summary status is `nonterminal_negative_probe` with train rows `80,000` and target rows `5,000`.
- Result: direct motion candidates changed mouse Pearson from prior direct-aux X/Y `-0.00447` / `null` to `0.01113` / `-0.01633`, but all paper-target gates except no-button FPR and nonzero button F1 still fail. Observed: keyboard key accuracy `0.01951`, mouse-button accuracy `0.00267`, mouse-button F1 `0.00556`, no-button FPR `0.04344`.
- Do not checkpoint `G005-g014-idm-full-paper-target`. This confirms prediction-only reranking over the raw96 patch-gated checkpoint is insufficient; next G005 work should train an explicitly stronger FDM-1-public-recipe IDM path instead of more threshold-only sweeps: raw video-token encoder + temporal masked action-token diffusion + train-heldout calibration, with better action-token temporal alignment and motion/token auxiliary supervision.

## 2026-05-29T13:45 KST — Next G005 branch: rank-trained sparse action-token confidence heads

- Implemented the next recipe-faithful IDM branch after prediction-only direct-aux/direct-motion sweeps remained far below D2E paper targets. The new branch keeps the public FDM-1 shape: raw screen-video patch tokens + noncausal temporal masked action-token diffusion + 16-step iterative unmasking.
- New approximation: pairwise confidence-ranking auxiliary losses on fit/train key, mouse-button, and mouse-move token-presence heads. This targets the observed failure that true key/button candidates exist but rank very low, and mouse-delta exact candidates are not recovered by the generic token-presence head.
- New dedicated mouse-move token-presence head ranks the same discrete FDM-1 binned mouse-delta action tokens used by the masked diffusion objective. This is not a continuous mouse regression shortcut and does not use target labels for calibration.
- New config/script: `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_rankaux_prefix80k.yaml` and `scripts/run_g005_idm_temporal_raw96_patch_rankaux_prefix80k.sh`. This requires a fresh training run; do not use it as a prediction-only override from the old checkpoint.
- Local validation: py_compile, bash syntax, `uv run pytest -q tests/test_masked_diffusion_idm_trainer.py tests/test_fdm1_recipe_alignment.py`, and `uv run python scripts/validate_fdm1_recipe_alignment.py` all pass. Do not checkpoint G005 unless H200 evidence beats paper-target gates.

## 2026-05-29T14:15 KST — G005 rank-aux raw96 patch-token probe is negative

- Ran commit/config `9271f07` / `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_rankaux_prefix80k.yaml` on production reservation `rsv-jeonghunpark-20260529-234edd`, pod `prod-rsv-jeonghunpark-20260529-234edd`, node4 GPU1. The reservation was cancelled after evidence copy.
- Evidence prefix: `artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_rankaux_prefix80k_h200_*`; compact summary status is `nonterminal_negative_probe` with train rows `80,000`, fit rows `78,000`, and target rows `5,000`.
- Result: rank-trained sparse token heads kept no-button FPR under cap (`0.02059`) but collapsed button/mouse emissions: keyboard key accuracy `0.01437`, mouse-button accuracy/F1 `0.0` / `0.0`, mouse Pearson X/Y `null` / `null`. Only the no-button FPR gate passed. Do not checkpoint `G005-g014-idm-full-paper-target`.
- Diagnosis: pairwise auxiliary ranking over the current raw96 patch-token checkpoint/config suppresses emissions too strongly and does not solve core video-action alignment. Next branch should not keep stacking candidate-ranking heuristics; move the FDM-1 recipe approximation closer to data/temporal alignment: verify D2E action/frame offset convention and train/evaluate offset-ensemble or multi-offset IDM heads within masked action-token diffusion before another full prefix run.
- Diagnostic detail: rank-aux improved exact key candidate coverage (`target_prefix` exact rate `0.976`) but not rank enough for paper accuracy (`p50` exact rank `108`), mouse-button exact candidates still rank near the bottom (`p50` `179`), and mouse-move exact candidate coverage remains `0.0`. This points to action-token/frame-offset alignment or tokenization mismatch more than another threshold-calibration issue.

## 2026-05-29 KST — G005 axis-class token-ranking probe (negative)

- Implemented and ran the FDM-1-recipe-shaped axis/key token-class ranking branch at commit `5cdf65b`: `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_axisclass_prefix80k.yaml` keeps raw 96×96 patch video tokens, noncausal temporal masked action-token diffusion, D2E metric-bin mouse-token adapter, and 16-step iterative unmasking. New key-token and mouse X/Y token-class heads are auxiliary confidence/ranking heads inside the masked action-token denoiser, not regression shortcuts or target-label calibration.
- Reservation: `rsv-jeonghunpark-20260529-673562`, production node 4 GPU 1, managed Codex image, 2026-05-29 15:00–17:00 KST. W&B run: `https://wandb.ai/pjh6029-seoul-national-university/fdm-d2e-reproduction/runs/coo38865`. Reservation was cancelled immediately after evidence copy.
- Evidence prefix: `artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_prefix80k_h200_*`, output metrics under `outputs/idm_temporal_masked_diffusion_d2e_raw96_patch_axisclass_prefix80k/`, and sanitized reservation lifecycle artifacts under `artifacts/cluster/g005_axisclass_*`.
- Result is non-terminal negative: compact summary status `nonterminal_negative_probe`; only no-button FPR passed. Observed all-split paper-target metrics: keyboard key accuracy `0.015748031496062992`, mouse-button accuracy/F1 `0.0/0.0`, mouse-move Pearson X/Y `0.010293875865570632/0.001795393107606389`, no-button FPR `0.021618282890673256`.
- Diagnostic implication: adding class/ranking heads reduced no-button FPR and made mouse-move Pearson weakly positive versus the metric-bin negative probe, but exact key/button/mouse identity is still far below D2E paper targets. Do not checkpoint G005 complete. Next branch should keep the FDM-1 recipe but change the representation/learning signal more fundamentally (for example stronger video/action-token pretraining, true sequence diffusion over multi-token rows, or distillation from released G-IDM only after paper-target path is exhausted), not threshold-only calibration.

## 2026-05-29T23:20 KST — G005 real-video frozen-DINO prefix16k gate is negative; quota used then cancelled

- User confirmed quota increase approval; the active 4×H200 reservation `rsv-jeonghunpark-20260529-c93357` / pod `prod-rsv-jeonghunpark-20260529-c93357` was used for a bounded real-video DINO feature gate rather than another compact-luma-only probe.
- The first launch at `503ae94` showed idle H200s because the cv2 fallback sought every frame with `CAP_PROP_POS_MSEC`. Commit `5391069` switched cv2 fallback to sequential nearby decoding with coarse seeking only for large jumps; the pod was relaunched on that commit and real-video DINO materialization became fast (`16k` train in `26.17s`, `16k` target in `25.28s`). Commit `248f176` fixes the shared wrapper summary path so future override runs embed the active paper-metrics JSON instead of the default prefix320k metrics.
- Terminal evidence: `artifacts/idm/g005_idm_frozen_frame_embedding_realvideo_prefix16k_compact_summary.json`, `artifacts/idm/g005_idm_frozen_frame_embedding_realvideo_prefix16k_paper_metrics.json`, repaired `artifacts/idm/g005_idm_frozen_frame_embedding_realvideo_prefix16k_run_summary.json`, materialization summaries/monitors, W&B sidecar `eoogvkyp`, and reservation lifecycle artifacts under `artifacts/cluster/g005_realvideo_prefix16k_*`.
- Result is **non-terminal negative**: all-split paper-compatible metrics on the 16k temporal target prefix were keyboard key accuracy `0.000368`, mouse-button accuracy `0.0`, mouse Pearson X/Y `0.00946/0.00371`; strict local no-button FPR was `0.0` only because the model emitted no buttons. Heldout-recording/game rows are absent in this bounded prefix. Do **not** checkpoint `G005-g014-idm-full-paper-target`.
- Reservation was cancelled after evidence copy (`artifacts/cluster/g005_realvideo_prefix16k_cancel_20260529.json`) to avoid idle 4×H200 time. Next branch should keep real-video/FDM-1 recipe alignment but fix the representation/objective more fundamentally before larger quota-backed runs: either train a masked-diffusion IDM directly on real video tokens instead of frozen DINO + generic streaming head, or use released G-IDM as a teacher only after the paper-target path is exhausted and clearly separated.

## 2026-05-29T23:55 KST — G005 balanced real-video raw96 prefix32k is terminal negative; larger cached branch prepared
...
## 2026-05-31T21:30 KST — G005 teacher-motion distillation full 4×H200 probe is negative

- Implemented and ran `g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_realvideo_statectx_teacher_motiondistill_train320k_target24k` at `bed8ad1` on production reservation `rsv-jeonghunpark-20260531-c52bf1`, pod `prod-rsv-jeonghunpark-20260531-c52bf1`, 4×H200. The branch preserved the public FDM-1 shape: raw96 video patch/state-context tokens, noncausal masked action-token diffusion, iterative unmasking, and train-only teacher-token distillation for mouse-motion tokens. The branch reused the audited state-context raw-video feature cache to avoid duplicate materialization.
- Terminal evidence copied locally under `artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_realvideo_statectx_teacher_motiondistill_train320k_target24k_*` plus `artifacts/idm/g005_event_state_duration_context_teacher_train320k_predict_summary.json`. Reservation cancellation evidence: `artifacts/cluster/g005_teacher_motiondistill_reservation_cancel_response.json`.
- Result is **non-terminal negative**: compact summary status `nonterminal_negative_probe`; all-split observed paper-target metrics are keyboard key accuracy `0.16015668718522663`, mouse-button accuracy/F1 `0.45614035087719296/0.6220570012391574`, mouse Pearson X/Y `0.6162087348192268/0.6686832764520063`, and no-button FPR `0.0`. Gates passed only for mouse-button F1 nonzero and no-button FPR; keyboard/button accuracy and mouse Pearson remain below paper targets. Do not checkpoint `G005-g014-idm-full-paper-target`.
- Split details: heldout-game mouse motion is strong (`0.9280/0.9203`) and button F1 is high (`0.8452`), but heldout-game keyboard is near zero (`0.00247`); heldout-recording and temporal keyboard remain `0.2823` and `0.2389`. This suggests train-only teacher distillation helped sparse button precision/recall and some motion splits, but key-token action semantics/generalization are still broken.
- Next branch should not weaken into target-label calibration or arbitrary supervised shortcuts. Keep the FDM-1 recipe and pivot to stronger action-token modeling: likely row/action-family multi-token sequence diffusion with explicit key press/release/state-change tokenization and/or a train-only released-G-IDM distillation phase for key/button tokens, followed by a split-safe paper-target evaluation. Also fix rank0-only post-training prediction before another large 4×H200 run if prediction rows exceed bounded 24k probes.
## 2026-05-31 KST — G005 warm teacher-motion-distillation negative probe

- Warm-started from the best prior state-context masked-diffusion IDM checkpoint and fine-tuned the FDM-1-recipe IDM branch with low-weight mouse-motion teacher distillation on 4×H200 (`rsv-jeonghunpark-20260531-c5cecf`, pod `prod-rsv-jeonghunpark-20260531-c5cecf`, repo `59266bd`).
- Terminal evidence is `artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_realvideo_statectx_warm_teacher_motiondistill_train320k_target24k_h200_compact_summary.json`; status is `nonterminal_negative_probe`, not G005 completion. Observed all-split paper-compatible metrics: keyboard `0.5093129139072847`, mouse-button acc `0.5481611208406305`, mouse-button F1 `0.7177700348432056`, mouse Pearson X/Y `0.6073438927715639` / `0.6464847430238749`, no-button FPR `0.0`. It passed only F1/non-FPR gates and failed keyboard, button accuracy, Pearson X/Y targets.
- Compared to the scratch teacher-motion-distillation branch, warm-start recovered keyboard and F1, but it still regressed the earlier state-context checkpoint on mouse-button acc and mouse Pearson; teacher-motion distillation should be treated as a negative branch unless reworked.
- Reservation was cancelled after terminal evidence; see `artifacts/cluster/g005_warm_teacher_motiondistill_reservation_postcancel_detail.json` (`status=cancelled`).
- Next recipe-faithful direction: do not keep increasing teacher-distill weight. Prioritize improving IDM metric gates through masked-diffusion decoding/objective changes that preserve the FDM-1 recipe: better iterative confidence unmasking and split-aware calibration from heldout calibration rows, stronger keyboard/button candidate budgets without target-label calibration, and/or larger recipe-aligned train rows only after the objective/decoder is corrected.

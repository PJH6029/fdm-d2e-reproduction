...
## 2026-05-31 KST — count-head prefix32k probe rejected

Bounded 4xH200 count-head masked-IDM probe ran on reservation `rsv-jeonghunpark-20260531-3b895d` (Node 6 GPU 0-3) from commit `b427ea8`. To avoid idle GPU materialization, it reused the byte-identical existing real-video balanced prefix32k train/target files via hardlinks and then trained the FDM-1-recipe masked action-token IDM with row-local family-count auxiliary heads. Terminal compact summary: `artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_realvideo_counthead_prefix32k_h200_compact_summary.json` status `nonterminal_negative_probe`. Observed paper-compatible all-split metrics: keyboard key accuracy `0.008574592627748774`, mouse-button accuracy `0.0`, mouse-button F1 `0.0`, mouse-move Pearson X `-0.01764905334025371`, Pearson Y unavailable, no-button FPR `0.0`. GPU monitor covered all 4 GPUs with max utilization `57%`; W&B sidecar completed (`run_id=6cvvv1w7`). Reservation was cancelled after negative result; no G005 completion claim. This rejects the current count-gated candidate as under-selecting sparse actions rather than solving the earlier overfire problem. Next recipe-faithful pivot should preserve masked diffusion but change the discrete action-tokenization/training target (for example explicit key/button release-state/trajectory tokens or diffusion over count-conditioned top-k token sets) instead of adding more global thresholds.

## 2026-05-31 KST — trajectory-duplicate masked IDM branch prepared

After the count-head probe under-selected sparse actions, the next recipe-faithful pivot targets a concrete tokenization/decoding bug: D2E 50ms bins often contain mouse trajectories with repeated binned movement tokens, but the previous candidate decoder collapsed duplicate action tokens and constrained mouse-move output to at most one X and one Y token per row. This makes paper-compatible mouse scale/Pearson structurally hard to match. Added duplicate-preserving candidate decoding for configured families (`candidate_duplicate_families=["mouse_move"]`) so repeated mouse movement tokens from distinct masked slots survive the FDM-1-style iterative unmasking budget. Prepared bounded probe config `configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_axisclass_realvideo_trajectorydup_prefix32k.yaml` and wrapper `scripts/run_g005_idm_temporal_raw96_patch_axisclass_realvideo_trajectorydup_prefix32k.sh`; it disables one-X/one-Y axis constraint and raises mouse trajectory budget to 16 tokens/row while preserving no target-label calibration. Validation: py_compile, wrapper `bash -n`, targeted pytest (`80 passed`), and `scripts/validate_fdm1_recipe_alignment.py` pass. Run only as a bounded prefix gate before any G005 promotion.

## 2026-05-31 KST — trajectory-duplicate prefix32k probe rejected

Bounded 4xH200 trajectory-duplicate masked-IDM probe ran on reservation `rsv-jeonghunpark-20260531-d65cec` (Node 6 GPU 0-3) from commit `075a91e`. It reused the existing real-video balanced prefix32k train/target files via hardlinks, then trained the FDM-1-recipe noncausal masked action-token IDM with duplicate-preserving mouse-move candidate decoding and a raised trajectory token budget.

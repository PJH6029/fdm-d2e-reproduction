...
MODEL_SLUG="${MODEL_SLUG:-g005_idm_temporal_masked_diffusion_raw96_patch_adaptive_prefix80k}"
CONFIG="${CONFIG:-configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_adaptive_prefix80k.yaml}"
SOURCE_CHECKPOINT="${SOURCE_CHECKPOINT:-outputs/idm_temporal_masked_diffusion_d2e_raw96_patch_gate_prefix80k/checkpoint.pt}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/idm_temporal_masked_diffusion_d2e_raw96_patch_adaptive_prefix80k}"
...
if command -v nvidia-smi >/dev/null 2>&1; then
  nvidia-smi --query-gpu=timestamp,index,name,utilization.gpu,utilization.memory,memory.used,memory.total,power.draw --format=csv -l 10 >"$GPU_MONITOR_LOG" 2>&1 &
  MONITOR_PID="$!"
...
payload={
  'schema':'g005_temporal_raw96_adaptive_predict_h200_run.v1',
  ...
  'claim_boundary':'Prediction-only adaptive-threshold sweep from a bounded raw-video FDM-1-recipe IDM checkpoint; not G005 completion evidence unless paper targets are beaten and audited.'
}
run_summary.parent.mkdir(parents=True, exist_ok=True)
run_summary.write_text(json.dumps(payload, indent=2, sort_keys=True)+'\n')
compact={
  'schema':'g005_temporal_raw96_adaptive_predict_compact_summary.v1',
  ...
  'claim_boundary':'Prediction-only compact evidence. G005 remains incomplete unless status=paper_target_pass and separate completion/goal checkpoint gates pass.'
}

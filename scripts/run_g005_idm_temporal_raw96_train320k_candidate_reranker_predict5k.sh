...
MODEL_SLUG="${MODEL_SLUG:-g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_realvideo_train320k_candidate_reranker_predict5k}"
...
CONFIG="${CONFIG:-configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_axisclass_realvideo_train320k_candidate_reranker_predict5k.yaml}"
...
WANDB_TAGS="${WANDB_TAGS:-g005,idm,d2e,fdm1-recipe,raw96,train320k,candidate-reranker,prediction-only}"
...
compact={
  'schema':'g005_candidate_reranker_prediction_compact.v1',
  ...
  'claim_boundary':'Prediction-only candidate-score reranker diagnostic from a prior G005 train320k checkpoint; train-heldout calibration only, no target-label calibration, not G005 completion evidence.'
}

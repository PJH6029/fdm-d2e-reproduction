...
MODEL_SLUG="${MODEL_SLUG:-g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_realvideo_statectx_public49_train320k_stratcal_target24k}"
CONFIG="${CONFIG:-configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_axisclass_realvideo_statectx_public49_train320k_stratcal_target24k.yaml}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/idm_temporal_masked_diffusion_d2e_raw96_patch_axisclass_realvideo_statectx_public49_train320k_stratcal_target24k}"
SOURCE_INPUT_ROOT="${SOURCE_INPUT_ROOT:-outputs/data/d2e_event_state_duration_context_shards_accel64}"
# Reuse the same balanced state-context rows and raw-video feature cache as the 320k statectx branch.
# Only the action-token target changes from D2E metric mouse bins to public FDM-1-style 49-bin mouse tokens.
...
PROCESS_PATTERN="${PROCESS_PATTERN:-train_idm_temporal_masked_diffusion|run_g005_idm_temporal_raw96_patch_axisclass_realvideo_statectx_public49_train320k_stratcal_target24k|run_g005_idm_temporal_raw96_family_presence_prefix}"
WANDB_TAGS="${WANDB_TAGS:-g005,idm,d2e,fdm1-recipe,public49,real-video,raw96,axisclass,state-context,balanced-prefix,train320k,stratified-calibration,distributed-feature-cache}"

...
CONFIG="${CONFIG:-configs/model/idm_streaming_d2e_full_event_state_duration_hierarchical_prefix320k.yaml}"
PAPER_CONFIG="${PAPER_CONFIG:-configs/eval/g005_idm_event_state_duration_hierarchical_prefix320k_paper_metrics.yaml}"
...
TRAIN_PREFIX_SUMMARY="${TRAIN_PREFIX_SUMMARY:-artifacts/idm/g005_idm_event_state_duration_hierarchical_prefix320k_train_materialization_summary.json}"
TARGET_PREFIX_SUMMARY="${TARGET_PREFIX_SUMMARY:-artifacts/idm/g005_idm_event_state_duration_hierarchical_prefix320k_target_materialization_summary.json}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/idm_streaming_d2e_full_event_state_duration_hierarchical_prefix320k}"
RUN_SUMMARY="${RUN_SUMMARY:-artifacts/idm/g005_idm_event_state_duration_hierarchical_prefix320k_run_summary.json}"
GPU_MONITOR_LOG="${GPU_MONITOR_LOG:-artifacts/idm/g005_idm_event_state_duration_hierarchical_prefix320k_gpu_monitor.csv}"
GPU_MONITOR_PID_FILE="${GPU_MONITOR_PID_FILE:-outputs/cluster/g005_hierarchical_prefix_gpu_monitor.pid}"
WANDB_SIDECAR_STATUS="${WANDB_SIDECAR_STATUS:-artifacts/idm/g005_idm_event_state_duration_hierarchical_prefix320k_wandb_sidecar_status.json}"
WANDB_SIDECAR_LOG="${WANDB_SIDECAR_LOG:-artifacts/idm/g005_idm_event_state_duration_hierarchical_prefix320k_wandb_sidecar.log}"
WANDB_SIDECAR_PID_FILE="${WANDB_SIDECAR_PID_FILE:-outputs/cluster/g005_hierarchical_prefix_wandb_sidecar.pid}"
...
uv run python scripts/materialize_chronological_prefix.py \
  ...
  --source-label "g005_event_state_duration_hierarchical_train_prefix320k"
uv run python scripts/materialize_chronological_prefix.py \
  ...
  --source-label "g005_event_state_duration_hierarchical_target_prefix320k"
if [[ "$ENABLE_WANDB_SIDECAR" != "0" ]]; then
  uv run --with wandb python scripts/watch_wandb_training.py \
    ...
    --run-name "g005-hierarchical-exactset-prefix320k" \
    --group "g005-idm-paper-target" \
    --job-type "train-sidecar" \
    --tags "g005,idm,d2e,hierarchical-exactset,prefix" \
    --poll-seconds 30 \
    --process-pattern "train_idm_streaming|torchrun|run_g005_idm_event_state_duration_hierarchical_prefix" \
...
summary={
  'schema':'g005_hierarchical_exactset_prefix_run_summary.v1',
  'status':'pass',
  ...
  'paper_metrics':json.load(open('artifacts/idm/g005_idm_event_state_duration_hierarchical_prefix320k_paper_metrics.json')),
  'gpu_monitor_log':'$GPU_MONITOR_LOG',
  'wandb_sidecar_status':'$WANDB_SIDECAR_STATUS',
  'claim_boundary':'Hierarchical exact-set event-head prefix diagnostic only; not G005 completion evidence.'
}

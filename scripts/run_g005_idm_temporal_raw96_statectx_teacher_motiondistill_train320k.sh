...
MODEL_SLUG="${MODEL_SLUG:-g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_realvideo_statectx_teacher_motiondistill_train320k_target24k}"
CONFIG="${CONFIG:-configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_axisclass_realvideo_statectx_teacher_motiondistill_train320k_target24k.yaml}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/idm_temporal_masked_diffusion_d2e_raw96_patch_axisclass_realvideo_statectx_teacher_motiondistill_train320k_target24k}"
...
TEACHER_CONFIG="${TEACHER_CONFIG:-configs/model/idm_streaming_d2e_full_event_state_duration_context_teacher_train320k_predict.yaml}"
TEACHER_OUTPUT_DIR="${TEACHER_OUTPUT_DIR:-outputs/idm_streaming_d2e_full_event_state_duration_context_teacher_train320k_predictions}"
TEACHER_SUMMARY="${TEACHER_SUMMARY:-artifacts/idm/g005_event_state_duration_context_teacher_train320k_predict_summary.json}"
...
PROCESS_PATTERN="${PROCESS_PATTERN:-train_idm_temporal_masked_diffusion|run_g005_idm_temporal_raw96_statectx_teacher_motiondistill_train320k|run_g005_idm_temporal_raw96_family_presence_prefix}"
WANDB_TAGS="${WANDB_TAGS:-g005,idm,d2e,fdm1-recipe,real-video,raw96,state-context,teacher-motion-distill,balanced-prefix,train320k}"
...
mkdir -p "$SOURCE_PREFIX_ROOT" "$TEACHER_OUTPUT_DIR" artifacts/idm outputs/cluster "$OUTPUT_DIR"
...
if [[ ! -s "$TEACHER_OUTPUT_DIR/predictions.jsonl" || ! -s "$TEACHER_SUMMARY" ]]; then
  uv run --extra train python scripts/predict_idm_streaming.py \
    --config "$TEACHER_CONFIG" \
    --prediction-workers "${TEACHER_PREDICTION_WORKERS:-1}" \
    --eval-batch-size "${TEACHER_EVAL_BATCH_SIZE:-4096}"
fi

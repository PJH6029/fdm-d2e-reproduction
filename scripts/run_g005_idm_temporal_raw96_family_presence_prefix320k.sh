...
CONFIG="${CONFIG:-configs/model/idm_temporal_masked_diffusion_d2e_raw96_family_presence_prefix320k.yaml}"
MODEL_SLUG="${MODEL_SLUG:-g005_idm_temporal_masked_diffusion_raw96_family_presence_prefix320k}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/idm_temporal_masked_diffusion_d2e_raw96_family_presence_prefix320k}"
...
PROCESS_PATTERN="${PROCESS_PATTERN:-train_idm_temporal_masked_diffusion|run_g005_idm_temporal_raw96_family_presence_prefix320k}"
...
if [[ "$ENABLE_WANDB_SIDECAR" != "0" && -n "${WANDB_PROJECT:-}" ]]; then
  uv run --with wandb python scripts/watch_wandb_training.py \
    ...
    --tags "g005,idm,d2e,fdm1-recipe,raw-video,masked-diffusion,prefix320k" \
    --poll-seconds 30 \
...
summary_path = pathlib.Path("artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_family_presence_prefix320k_summary.json")
...
payload={
  'schema':'g005_temporal_raw96_family_presence_prefix320k_h200_run.v1',
  ...
  'claim_boundary':'Bounded 1xH200/prefix320k raw-video FDM-1-recipe IDM scale probe; not G005 completion evidence unless paper targets are beaten and audited.'
}

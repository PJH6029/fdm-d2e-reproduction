...
MODEL_SLUG="${MODEL_SLUG:-g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_realvideo_statectx_warm_continue1m_target24k}"
CONFIG="${CONFIG:-configs/model/idm_temporal_masked_diffusion_d2e_raw96_patch_axisclass_realvideo_statectx_warm_continue1m_target24k.yaml}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/idm_temporal_masked_diffusion_d2e_raw96_patch_axisclass_realvideo_statectx_warm_continue1m_target24k}"
SOURCE_INPUT_ROOT="${SOURCE_INPUT_ROOT:-outputs/data/d2e_event_state_duration_context_shards_accel64}"
SOURCE_PREFIX_ROOT="${SOURCE_PREFIX_ROOT:-outputs/data/d2e_event_state_duration_realvideo_balanced_train1m_target24k}"
SOURCE_TRAIN_PREFIX_SUMMARY="${SOURCE_TRAIN_PREFIX_SUMMARY:-artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_realvideo_statectx_warm_continue1m_source_train_balanced_summary.json}"
SOURCE_TARGET_PREFIX_SUMMARY="${SOURCE_TARGET_PREFIX_SUMMARY:-artifacts/idm/g005_idm_temporal_masked_diffusion_raw96_patch_axisclass_realvideo_statectx_warm_continue1m_source_target_balanced_summary.json}"
MAX_TRAIN_ROWS="${MAX_TRAIN_ROWS:-1000000}"
MAX_TARGET_ROWS="${MAX_TARGET_ROWS:-24000}"
TRAIN_MAX_PER_RECORDING="${TRAIN_MAX_PER_RECORDING:-60000}"
TARGET_ROWS_PER_SPLIT="${TARGET_ROWS_PER_SPLIT:-8000}"
NPROC_PER_NODE="${NPROC_PER_NODE:-4}"
PROCESS_PATTERN="${PROCESS_PATTERN:-train_idm_temporal_masked_diffusion|run_g005_idm_temporal_raw96_statectx_warm_continue1m|run_g005_idm_temporal_raw96_family_presence_prefix}"
WANDB_TAGS="${WANDB_TAGS:-g005,idm,d2e,fdm1-recipe,real-video,raw96,state-context,warm-continue,train1m,stratified-calibration}"

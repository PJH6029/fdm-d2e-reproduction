...
Latest known live run snapshot: 2026-05-21 16:42 KST.
...
- Pod checkout contains finalizer hardening commits through `1124998` (`Make G008 live-suite evidence finalization explicit`); G003/G004/G005/G008 finalizers, split-stat generation, completion audits, and package-manifest updates are present in pod.
- Latest monitor artifact: `artifacts/idm/g003_full_compact_parallel_progress.json`.
- Last decoded count: `121 / 918` recording variants; shard summaries `0 / 16`; IDM metrics absent.
...
- Parent PID `9289` was still running at elapsed `05:44:41`; attached GPU monitor PID `31950` was still running at elapsed `01:55:48` and writing `artifacts/idm/g003_d2e_full_idm_4xh200_gpu_monitor.csv` in the pod. Do not commit/push that live CSV from local until the run exits, because the pod currently owns it as an untracked live output.

...
Latest known live run snapshot: 2026-05-21 16:01 KST.
...
- Pod checkout contains the split-stat and manifest hardening commits through `2c4c3fc` (`Keep final audit covered by reproducibility manifest`); G003/G004 split-stat generation, G005/G006/G007/G008/G009 completion audit scripts, and package-manifest updates are present in pod.
...
- Parent PID `9289` was still running at elapsed `05:06:51`; attached GPU monitor PID `31950` was still running at elapsed `01:17:57` and writing `artifacts/idm/g003_d2e_full_idm_4xh200_gpu_monitor.csv` in the pod. Do not commit/push that live CSV from local until the run exits, because the pod currently owns it as an untracked live output.
